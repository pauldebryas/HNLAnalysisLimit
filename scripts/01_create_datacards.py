...
for chn in config['GENERAL']['channels']:
    ...
    var_name = config["DV"][chn][var_nb]
    for mass in masses:
        filepath = '{}/{}/{}/FakeRate/SignalRegion/TH1_{}_HNLMass{}_HNL.root'.format(
            era, config["GENERAL"]["tag"], chn, var_name, mass)
        file = os.path.join(config['PATH']['thf1hist_files_dir'], filepath)
        print(file)
        cb_copy.cp().channel([chn]).era([era]).backgrounds().mass([mass]).ExtractShapes(
            file, '$BIN/$PROCESS', '$BIN/$PROCESS_$SYSTEMATIC')
        cb_copy.cp().channel([chn]).era([era]).signals().mass([mass]).ExtractShapes(
